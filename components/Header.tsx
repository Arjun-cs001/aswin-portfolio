        ...
        <div className="w-full max-w-5xl bg-black/80 backdrop-blur-xl border border-white/10 rounded-full px-4 py-3 flex items-center justify-between shadow-2xl relative">
          ...
          <div className="flex items-center gap-3">
             <div className="relative group">
                <div className="absolute -inset-0.5 bg-[#C0182A] rounded-full blur opacity-30 group-hover:opacity-75 transition duration-200"></div>
                 <img 
                   src="https://res.cloudinary.com/dcnz8e0nz/image/upload/v1775791063/profile2_ag2rmb.png" 
                   alt="Aswin GFX" 
                   className="relative w-8 h-8 sm:w-10 sm:h-10 rounded-full border-2 border-[#C0182A]/40 object-cover object-center bg-zinc-800"
                   onError={(e) => {
                     const target = e.target as HTMLImageElement;
                     target.src = "https://ui-avatars.com/api/?name=Aswin+GFX&background=C0182A&color=fff&rounded=true&bold=true";
                   }}
                 />
             </div>
          </div>
          ...
          <div className="flex items-center gap-4 sm:gap-6">
              ...
              <Button variant="primary" onClick={onOpenOrder} className="rounded-full px-5 py-2 sm:px-8 sm:py-2.5 text-[10px] sm:text-xs uppercase tracking-widest hover:-translate-y-0.5">
               ...
               <div className="md:hidden">
                  <button onClick={() => setIsOpen(!isOpen)} className="text-white hover:text-[#C0182A]">
